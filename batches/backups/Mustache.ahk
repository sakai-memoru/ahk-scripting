...
class Mustache {
    ...
    static Compile(template) {
        ...
        reader := Mustache.Reader(template)
        compiler := Mustache.Compiler()
        ret := compiler.Compile(reader, startMarker, endMarker, "", false, true)
    ...
    class Compiler {
        ...
        Compile(reader, startMarker, endMarker, tagName := ""
            ...
            while((char := reader.Read()) || char == 0) {
                ...
                else if (! this.IsWhitespaceChar(char) && skipNextLinebreak) {
                    skipNextLinebreak := false
                }
                ...
                if (char == startMarker[1]) {
                    ...
                    if (reader.Read() != startMarker[2]) {
                        ...
                        if (!this.isCommand(commandTagType)) {
                            ...
                            textBuffer := this.WriteTextBuffer(&tokens, textBuffer)
                            tokens.Push(this.VariableToken(command))
                            ...
                            if (commandTagType == "&" || commandTagType == "{") {
                                ...
                                textBuffer := this.WriteTextBuffer(&tokens, textBuffer)
                                tokens.Push(this.EscapedVariableToken(commandTagName))
                            }
                            ...
                            if (commandTagType == "#") {
                                ...
                                textBuffer := this.WriteTextBuffer(&tokens, textBuffer)
                                tokens.Push(this.SectionToken(commandTagName, childTokens))
                            ...
                            if (commandTagType == "^") {
                                ...
                                textBuffer := this.WriteTextBuffer(&tokens, textBuffer)
                                tokens.Push(this.InvertedToken(commandTagName, childTokens))
        ...
        WriteTextBuffer(&tokens, textBuffer) {
            if (textBuffer) {
                tokens.Push(this.TextToken(textBuffer))
            }
        ...
        IsStandaloneTag(whitespaceBefore, reader) {
            ...
            while ((char := reader.Read()) || char == 0) {
                if (!this.IsWhitespaceChar(char) && char != this.linebreakChar) {
        ...
        Parse(writer, tokens, globalHash, scopedHash := "") {
            for _, thisToken in tokens {
                ...
                if (tokenType == "V" || tokenType == "&") {
                    ...
                    if (this.IsFloat(hashValue)) {
                        hashValue := this.TrimFloat(hashValue)
                    }
                ...
                if (tokenType == "S") {
                    ...
                    if (isObjectValue) {
                        isArrayValue := this.IsArray(hashValue)
                    }
                ...
                if (tokenType == "E") {
                    ...
                    if (isObjectHash)
                        isArrayHash := this.IsArray(scopedHash)
                    ...
                    if (this.IsFloat(scopedHash)) {
                        scopedHash := this.TrimFloat(scopedHash)
                    }
